...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-XUEZ"
  !define PRODUCT_WEB_SITE "https://github.com/ddude1/electrum-xuez"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-xuez-${PRODUCT_VERSION}-setup-${BUILD_ARCH}.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "icons\electrum-xuez.ico"
...
Section
  ...
  File /r "dist\electrum-xuez\*.*"
  File "icons\electrum-xuez.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-xuez-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-xuez-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-xuez-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-xuez-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-xuez-${PRODUCT_VERSION}.exe" 0


  ;Links xuez: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\xuez" "" "URL:xuez Protocol"
  WriteRegStr HKCU "Software\Classes\xuez" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\xuez" "DefaultIcon" "$\"$INSTDIR\electrum-xuez.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\xuez\shell\open\command" "" "$\"$INSTDIR\electrum-xuez-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-xuez.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\xuez"
