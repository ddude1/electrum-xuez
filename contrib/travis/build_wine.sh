...
cd $WINEPREFIX/drive_c/electrum-xuez

rm -rf build
rm -rf dist/electrum-xuez
...
wine pip install git+git://github.com/ddude1/Xevan_in_Python.git
...
wine "$NSIS_EXE" /NOCD -V3 \
    /DPRODUCT_VERSION=$ELECTRUM_XUEZ_VERSION \
    /DWINEARCH=$WINEARCH \
    contrib/build-wine/electrum-xuez.nsi
