...
try:
    ...
    from electrum_xuez.bitcoin import TYPE_ADDRESS, push_script, var_int, msg_magic, Hash, verify_message, pubkey_from_signature, point_to_ser, public_key_to_p2pkh, EncodeAES, DecodeAES, MyVerifyingKey
    from electrum_xuez.bitcoin import serialize_xpub, deserialize_xpub
    from electrum_xuez.transaction import Transaction
    from electrum_xuez.i18n import _
    from electrum_xuez.keystore import Hardware_KeyStore
    from ..hw_wallet import HW_PluginBase
    from electrum_xuez.util import print_error, to_string, UserCancelled
...
class DigitalBitbox_Client():
    ...
    def dbb_generate_wallet(self):
        ...
        filename = ("Electrum-XUEZ-" + time.strftime("%Y-%m-%d-%H-%M-%S") + ".pdf").encode('utf8')
        msg = b'{"seed":{"source": "create", "key": "%s", "filename": "%s", "entropy": "%s"}}' % (key, filename, b'Digital Bitbox Electrum-XUEZ Plugin')
