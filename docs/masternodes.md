...
Electrum-XUEZ supports masternode creation through an interface called the Masternode Manager.
...
- The XUEZ address that received the collateral payment.
- The XUEZ address of the delegate key.
...
A masternode requires a "delegate" key, which is known to both Electrum-XUEZ and your masternode.
Your masternode will use this key to sign messages, and the XUEZ network will know that you authorized
it to.

A delegate key can either be one of your Electrum-XUEZ keys, or an imported key. Either way, your masternode
and Electrum-XUEZ will both need to know the private key. (See *Importing Masternode.conf* below.)

To use one of your Electrum-XUEZ keys as a delegate key, put its address in the `Masternode XUEZ Address`
...
### Collateral (1000 XUEZ Payment)

To start a masternode, you must have a 1000 XUEZ payment available in your wallet.
You can scan your wallet for 1000 XUEZ payments in the `Activate Masternode` tab of the Masternode
Manager.

After scanning, a list of available 1000 XUEZ collateral payments will be displayed. Selecting one
...
waiting for Electrum-XUEZ to sign and broadcast your masternode announcement, you will be presented with
