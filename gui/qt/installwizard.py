...
from electrum_xuez import Wallet, WalletStorage
from electrum_xuez.util import UserCancelled, InvalidPassword
from electrum_xuez.base_wizard import BaseWizard
...
MSG_GENERATING_WAIT = _("Electrum-XUEZ is generating your addresses, please wait...")
...
class InstallWizard(QDialog, MessageBoxMixin, BaseWizard):
    ...
    def __init__(self, config, app, plugins, storage):
        ...
        self.setWindowTitle('Electrum-XUEZ  -  ' + _('Install Wizard'))
        self.app = app
        self.config = config
        ...
        self.set_icon(':icons/electrum-xuez.png')
        self.show()
        self.raise_()
    ...
    def run_and_get_wallet(self):
        ...
        self.set_layout(vbox, title=_('Electrum-XUEZ wallet'))
        ...
        if self.storage.requires_split():
            self.hide()
            msg = _("The wallet '%s' contains multiple accounts, which are no longer supported in Electrum-XUEZ 2.7.\n\n"
        ...
        if self.storage.requires_upgrade():
            self.hide()
            msg = _("The format of your wallet '%s' must be upgraded for Electrum-XUEZ. This change will not be backward compatible"%path)
            if not self.question(msg):
                return
    ...
    def init_network(self, network):
        message = _("Electrum-XUEZ communicates with remote servers to get "
                  "information about your transactions and addresses. The "
                  "servers all fulfil the same purpose only differing in "
                  "hardware. In most cases you simply want to let Electrum-XUEZ "
